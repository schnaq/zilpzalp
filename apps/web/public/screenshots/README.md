...
| `05-album.png` | Album with several stickers found |

The screens come from issue #168, whose sixth capture — the rank ladder — is
gone with the ladder itself (decision of 2026-09-09). Rename the iPhone set
(6.9", 1320×2868) that `mise run screenshots` writes to
`DerivedData/screenshots/<device>/` as above. Any aspect ratio works, the frames
crop to portrait, but keep the five consistent with one another.
