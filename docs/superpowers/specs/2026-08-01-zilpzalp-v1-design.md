...
**ZilpZalpUI** übersetzt das Design-System nach SwiftUI: Farb- und Typo-Tokens, `ZButton`, `ZCard`, `ChoiceTile`, `SoundButton`, `QuizProgress`, `FeedbackBanner`, `RewardSticker`, `HomeTile`, `SettingRow`. Nach den Komponenten unter `design/components/`, mit bewussten Abweichungen wie unten vermerkt. Komponenten bekommen fertige `String`-Werte übergeben und tragen selbst keine Produkttexte. Gesperrte Sticker in der Sammlung bleiben sichtbar — mit Schloss-Symbol statt versteckt, und darunter zeigt `StickerProgress` in fünf Markierungen, wie weit dieser Vogel gekommen ist (Entscheidung 2026-09-09, Issue #177). `HomeTile` kennt einen gesperrten Zustand mit Ei-Symbol als Komponentenfähigkeit, aber v1 setzt ihn auf dem Startbildschirm nicht ein: der besteht nur aus `TopBar` und den zwei Kacheln für Spiel 1 und Spiel 2, ohne Baum und ohne gesperrte Nester für Spiel 3 und 4. Dynamic Type ist bewusst fest: die Geometrie (220 pt Kacheln, 64/96/160 pt Bedienziele) skaliert nicht mit dem Text (Entscheidung 2026-09-07). Das gilt unverändert für alles, was ein Kind sieht; der Elternbereich folgt dagegen seit 2026-09-11 der Systemschriftgröße (Entscheidung 2026-09-11, Issue #239). Einstellungen, „Über ZilpZalp", Credits und das Parental Gate sind Fließtext und Zeilen in einem Scroll-View ohne die feste Geometrie der Spielbildschirme, und Eltern sind genau die Leser, die größere Schrift eingestellt haben. Gedeckelt bei `accessibility3`, die Schriftschnitte bleiben, jede Stufe skaliert relativ zu einem eigenen `Font.TextStyle`, und `SettingRow` stapelt ab einer Accessibility-Größe auch auf dem iPad. Die Textfarben halten den WCAG-Kontrast (SC 1.4.3): `ContrastTests` rechnet für jedes Text/Hintergrund-Paar, das die Komponenten zeichnen, das Verhältnis aus den Tokens selbst aus — 4,5:1, und 3:1 erst ab 24 pt fett. Zwei Paare lagen darunter und sind korrigiert: die Sonnen-Kachel schreibt jetzt in `ink900` statt in `sun600` (2,87:1 → 12,75:1), und `orange700` ist als einziger Rampenwert gegenüber `colors.css` abgedunkelt, von `#A64B12` auf `#9C4410` (4,22:1 → 4,73:1) — es ist die einzige orange Farbe, in der das System Wörter schreibt, nämlich auf der Kachel von Spiel 1 und in `Badge`. Nicht abgedunkelt sind `clay500` und `orange500`: auf beiden steht nie Text (`ZButton` kennt kein Clay, und eine Akzent-Schaltfläche mit Beschriftung zeichnet kein Screen), und ein Griff in die Rampe würde Marke, Fokusring und Rubriken mitverschieben. Rückmeldung ist außerdem fühlbar: `ChoiceTile` gibt über `.sensoryFeedback` bei der Antwort ein `success` und beim nochmal-Versuchen ein `warning` — nie `.error`, denn dieses Spiel kennt kein Falsch —, das Rundenende ein `success`, wenn ein neuer Sticker landet, und der Rundenstart ein `start`. Und das Rundenende sagt sich an: mit VoiceOver sprach der Bildschirm bisher gar nichts (am 2026-09-11 auf einem iPhone 17 Pro Max gemessen, mit Gegenprobe), jetzt kündigt er über `AccessibilityNotification.Announcement` Lob, Sternezahl und Erstfund an — und ersetzt dabei die eigene Sprachausgabe, damit nicht zwei Stimmen übereinander reden (Entscheidung 2026-09-11, Issue #238).
...
struct Bird: Codable, Identifiable {
    ...
    let article: String         // "die" — Artdatum; seit #220 spricht keine Zeile ihn
...
2. Zehn Fragen. Pro Frage: Aufgabe wird ausgegeben — Spiel 1 sagt den Vogelnamen (mit `pronunciation`-Override, wo hinterlegt), Spiel 2 spielt den Ruf ab, beides wiederholbar per Tippen
...
Das Kind wählt auf der Startseite, welche Vögel seine Runden fragen (Issue #187). Seit dem Paket-Download (#34) spielen alle installierten Pakete zusammen — mit Deutschland, Welt und Afrika sind das 131 Arten, und wer „die Vögel aus Afrika" wollte, konnte nicht danach fragen. Die Wahl ist ein Profilfeld (`collection`, leer heißt alle Vögel), gehört also dem Kind und wandert mit dem Profilwechsel; ein Paket, das im Elternbereich gelöscht wird, fällt stillschweigend auf „Alle Vögel" zurück, ohne die Wahl zu überschreiben. Auf der Startseite steht dafür **eine Pille** zwischen Überschrift und Spielkacheln — Foto der Kollektion und ihr Name, 64 pt hoch — und darüber öffnet sich die Auswahl als Karte über der abgedunkelten Startseite. Keine Reihe von Kollektionen: Fünf Einträge brauchen bei 96 pt Durchmesser 480 pt, ein 390-pt-iPhone lässt zwischen seinen Gutters 294 pt, und damit bliebe nur Scrollen (versteckte Inhalte für ein Kind, das nicht liest) oder Kreise unter der 64-pt-Grenze. Aus den Tokens gerechnet kostet die Pille auf einem 390-pt-iPhone rund 40 pt Kachelgröße und auf dem iPad nichts (auf einem 440-pt-iPhone und dem 13-Zoll-iPad im Simulator nachgesehen); beide Spiele bleiben überall ohne Scrollen sichtbar. Jeder Eintrag ist ein rundes Foto des ersten Vogels seines Manifests mit dem Titel darunter — „Alle Vögel" trägt das Vogel-Piktogramm, weil der erste Vogel der Bibliothek derselbe Amsel ist wie der des Basispakets —, der gewählte Eintrag trägt Ring **und** Häkchen, und ein Tippen spricht den Namen. Spiel 2 folgt der Wahl: Es erscheint nur, wenn die gewählte Kollektion mindestens vier Arten mit Ruf hat; eine Kollektion mit weniger als vier Arten wird gar nicht erst angeboten. Ist nur ein Paket installiert, gibt es nichts zu wählen und die Startseite sieht aus wie vorher. Sticker, Sterne und Sammlung bleiben global — die Kollektion entscheidet nur, wonach eine Runde fragt (Entscheidung 2026-09-10, Issue #187). Die Tür zur Sammlung am Fuß der Startseite ist eine olivgrüne Pille „Sammlung" mit Album-Symbol (`ZButton` in `md`, 64 pt, auf jedem Gerät — die einzige `md`-Pille auf einem Kinder-Screen) statt einer unbeschrifteten Scheibe — dasselbe Wort wie am Rundenende, damit die Startseite eine Sprache spricht; das „i" bleibt in seiner Ecke, denn die Pille braucht 192 pt und lässt auf einem 375-pt-iPhone 11,5 pt Luft bis zu ihm (in `ZilpZalpUI` gemessen, nicht geschätzt), keine Kachel zahlt dafür (auf dem Telefon ist die Reihe so hoch wie vorher, auf dem iPad stehen die Kacheln ohnehin auf der 240-pt-Rasterzelle, dort werden die 32 pt zu Luft) (Entscheidung 2026-09-10, Issue #212). Auf dem Telefon hält die Startseite denselben 16-pt-Gutter wie die `TopBar` seit #124 und nicht die 48 pt des iPads: Die iPad-Marge nahm ein Viertel eines 375-pt-Bildschirms und drückte zwei Kacheln auf 127 pt, wo „Wer singt da?" mit 115,1 pt auf der 20-pt-Untergrenze nicht mehr hineinpasste und abgeschnitten wurde; mit 16 pt ist die kleinste Kachel `(375 − 32 − 24) / 2 = 159` pt. Passt eine Beschriftung trotzdem nicht in eine Zeile, bricht sie in eine zweite, statt zu schrumpfen oder abgeschnitten zu werden; seit „Erkenne den Vogel" (#229) wird das auf einem 375-pt-iPhone gebraucht: Die Beschriftung von Spiel 1 braucht bei 20 pt 166,7 pt, die Kachel lässt 127 pt, und geschrumpft stünde sie auf 15,2 pt — unter der 20-pt-Untergrenze. Zweizeilig behält sie ihre Stufe: SwiftUI setzt sie ausgewogen als „Erkenne" über „den Vogel" (73,9 und 88,7 pt), und selbst der gierige Umbruch nach „den" bliebe mit 111,8 pt drin; Piktogramm, Abstand und zwei Zeilenkästen sind 126,1 pt der 127 pt, die die Kachel innen lässt. Die Schrumpf-Stufe (20 auf 16 pt) bleibt die letzte Reserve dahinter. Und die 240-pt-Kachel zeichnet ihre Beschriftung bewusst in `--text-headline` (28 pt), wo `HomeTile.jsx` 22 pt zeichnet: Ein kleines Wort unter einem 82-pt-Piktogramm sah verloren aus (Entscheidung 2026-09-10, Issues #145 und #92).
...
Spiel 1 sagt **nur den Vogelnamen** — „Zaunkönig", nicht „Wo ist der Zaunkönig?" —, und die `TopBar` der laufenden Runde trägt den Namen des Spiels (Entscheidung 2026-09-10, Christian, Issue #220). Der ganze Satz lief bei jeder der zehn Fragen durch, und was das Kind daraus braucht, ist ein Wort; der Rest ist Geräusch vor der Aufgabe. Gesprochen und geschrieben steht dasselbe: der Name, **ohne Artikel**. Der Artikel war eine eigene Fehlerquelle — die Vorlage setzte ihn vor den Namen und erzeugte „der Lachender Hans" —, und die artikellose Form umgeht sie, statt sie pro Art zu reparieren. `article` bleibt trotzdem im Manifest-Schema, weil es dort Artdaten sind und nicht Oberflächentext. Damit wird die gesprochene Zeile von Spiel 1 dieselbe wie die des Sammlungs-Stickers, also derselbe Schlüssel `collection.name` und dieselbe Aufnahme pro Art (`SpeechKey.speciesName`, Issue #221) — eine Aufnahme weniger pro Vogel und kein Satz, der beim Umformulieren aus dem Ruder läuft. Die Fragezeile von Spiel 2 verliert ihren Text ganz und behält den Abspiel-Knopf: Der Titel steht jetzt oben, und unter dem Knopf stand ohnehin nur derselbe Satz zweimal. Spiel 1 heißt dafür überall „Erkenne den Vogel" statt „Wer ist das?" — eine Sprache für Kachel und Bar, und die Aufforderung beschreibt, was das Kind tut, während „Wer ist das?" beschreibt, was Spiel 3 und 4 einmal tun werden. Der Name ist seit dem TestFlight-Build „Erkenne den Vogel" und nicht „Finde den Vogel": Das Kind sucht keinen versteckten Vogel, es erkennt den genannten wieder (Entscheidung 2026-09-10, Christian, Issue #229). Preis dieser Entscheidung: Die Beschriftung braucht bei 20 pt 166,7 pt und die schmalste Kachel eines 375-pt-iPhones lässt 127 pt, also nimmt sie dort zwei Zeilen — das erste Mal, dass eine Kachel-Beschriftung umbricht (gemessen in `ZilpZalpUI`, nicht geschätzt). In der `TopBar` passt sie einzeilig: 233,3 pt gegen 167 pt Mitte auf demselben Telefon, also 20,0 pt, genau auf der Untergrenze für alles, was ein Kind liest; auf einem 440-pt-Telefon sind es 27,9 pt und auf dem iPad die vollen 28. Dass die Bar diese Größe überhaupt zeichnet, ist ebenfalls #229: Der Titel stand auf jedem Gerät auf 18 pt, weil der `minimumScaleFactor` auch auf die enge Zeilenhöhe schrumpft und nicht nur auf die Breite. Die VoiceOver-Beschriftung des Knopfes sagt, was er sagen wird — in Spiel 1 mit Namen („Zaunkönig noch einmal hören"), in Spiel 2 ohne, denn dort ist der Name die Antwort.

Der Blätter-Fortschritt zeigt den Stand ohne Zahlen. Die Blätterreihe sitzt unter der TopBar statt wie im Design darin, auf jedem Gerät: Zehn 44-pt-Blätter mit 12 pt Abstand sind 548 pt breit — breiter als jedes iPhone —, und seit die Bar den Spielnamen trägt (#220) ist die Mitte auch auf dem iPad belegt; Titel und Reihe nebeneinander gingen auf einem 11-Zoll-iPad im Hochformat nicht auf. Bei erschöpftem Zeitbudget läuft die aktuelle Runde noch zu Ende, danach erscheint „Zeit fürs Nest".
...
**Sprachausgabe:** `AVSpeechSynthesizer` mit `de-DE` direkt auf dem Gerät. Kein Audio-Asset, keine Lizenzfrage, keine Netzabhängigkeit. Der Klickprototyp macht es mit der Web Speech API bereits genauso. Die Sprachausgabe ist standardmäßig an und lässt sich im Erwachsenenbereich abschalten: eine Schalter-Zeile „Ansagen vorlesen" mit dem Hinweis „Vogelnamen und Lob werden vorgelesen. Die Vogelrufe bleiben an." (`speechEnabled` in `ParentalSettings`, Symbol `volume-2`). Aus heißt still — auch die vorproduzierten Aufnahmen bleiben stumm, nicht nur der Synthesizer —, und zwar überall: Vogelname in Spiel 1, Lob am Rundenende, Stickername in der Sammlung, jede vorgelesene Überschrift. Der Abspiel-Knopf in Spiel 1 bleibt trotzdem stehen und bleibt bedienbar; er ist die Frage, und der Name steht daneben auf dem Bildschirm. Die Rufe von Spiel 2 rührt der Schalter nicht an — sie sind dort die Aufgabe. Das Kind kann damit ohne Vorlesen in Spiel 1 nur noch den geschriebenen Namen nutzen; deshalb ist die Voreinstellung „an" und die Entscheidung liegt hinter dem Schloss (Entscheidung 2026-09-10, Christian, Issue #231). Spiel 2 erscheint auf der Startseite nur, wenn es auch spielbar ist — mindestens vier Arten des Pakets mit Ruf; sonst fehlt seine Kachel, wie die von Spiel 3 und 4, statt gesperrt oder ohne Rufe angeboten zu werden (Issue #31). Einen Schalter „Vogelstimmen" gibt es nicht: Sind Rufe da, ist Spiel 2 da, und wer Ruhe will, nutzt die Gerätelautstärke (Entscheidung 2026-09-08, Issue #138). Die Audiosession (`AVAudioSession`, Kategorie `.playback`) wird vom Sprachdienst konfiguriert und vom Rufe-Player mitgenutzt, damit beide auch bei umgelegtem Stummschalter hörbar bleiben.
...
Settings/parental.json      Zeitbudget, Sprachausgabe an/aus, ob FaceID aktiv ist
```
